...
bool configCompare (double& src_temp, double& src_energy, double& target_temp, double& target_energy) {
...
bool Annealer::mpiSwap(int myrank, double temp_rank, double hami_rank, std::vector<Spin>& config) {
    ...
    if (myrank == src_a) {
        ...
        is_swap = configCompare(temp_rank, hami_rank, temp_other, hami_other);
...
double Annealer::anneal(std::tuple<double, double> temperature, Graph& graph) {
    ...
    for (int i = 0; i < tau; ++i) {
        ...
        if (i % 8 == 0) {
            ...
            // MPI communication
            mpiSwap(myrank, T, graph.getHamiltonianEnergy(), config);
        }
    }
